...
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");
  ...
  $this->load->helper('string');
		...
		if($select_data)
		{
						...
						$number=rand();
						$newpassword=$number;
						$Insert_pass = $this->db->query('update dev_users SET password="'.$newpassword.'" where email="'.$email.'" ');
						if($role=='3')
						{
						$Insert_pass_in_enginner = $this->db->query('update dev_engineer SET password="'.$newpassword.'" where email="'.$email.'" ');
						}
