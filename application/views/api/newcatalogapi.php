...
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");
  ...
  $this->load->helper('string');
	 ...
	 $fetchpdfs=$Business[0]['business_id'];
	  ...
	  $pdfdata=$this->db->query("SELECT DISTINCT dev_cetalog_section.id,dev_cetalog_section.section_name,dev_cetalog_section.cover_image,cetalouges.url from dev_cetalog_section INNER JOIN cetalouges on cetalouges.sectionid=dev_cetalog_section.id where dev_cetalog_section.business_id='$fetchpdfs'");
	 ...
	 { 
	    if(array_search($pdf['id'], array_column($emptyarray, 'id')) !== false)
	    {
	            ...
	            $emptyarray[$index]['catalog']=$pre.",".$pdf['url'];        
        }
        else {
           array_push($emptyarray,array('id'=>$pdf['id'],'section_name'=>$pdf['section_name'],'cover_image'=>$pdf['cover_image'],'catalog'=>$pdf['url']));
        }
