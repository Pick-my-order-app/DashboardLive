	<!-- end::Demo Panel -->
				...
				<!-- begin:: Footer -->
					<!--<div class="kt-footer kt-grid__item kt-grid kt-grid--desktop kt-grid--ver-desktop">
						...
						<div class="kt-footer__menu">
							...
							<a href="www.pickmyorder.co.uk" target="_blank" class="kt-footer__menu-link kt-link">2019&nbsp;&copy;&nbsp;Pick My orde</a>
						</div>
					</div>-->
		...
		<!--end::Page Scripts -->
		<?php
			if($_SERVER['SCRIPT_URL']!='/products'){
				if(isset($_SESSION['last_insert_id']))
				 {
					unset($_SESSION['last_insert_id']);
				 }
			}
			if($_SERVER['SCRIPT_URL']!='/Edit_single_product'){
		...
		<script>
			...
			jQuery('.select_business').change(function(){
			$business_id = $(this).val();
			
			$.ajax({
				url:"<?= base_url('')?>SetBusinessSection",
				method:"POST",
				data:{'CB':$business_id},
				...
				success:function(data){
					...
					if(data=='1')
					{
						...
						var url   = "<?= base_url('')?>dashboard";//window.location.href;
